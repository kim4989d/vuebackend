...
public class QuestionWebCmd    extends CommandFactory{
	private static final Logger logger = LoggerFactory.getLogger(BoardCmd.class);
	
	public QuestionWebCmd() {
	logger.info("QuestionWebCmd");

	setNextPage("QuestionWebCmd");
}
	...
	public ResultSetTray execute(Tray reqTray, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			/*
			 * QuestionDao dao=new QuestionDao(); rsTray=dao.QuestionList(getConnection(),
			 * reqTray);
			 */
